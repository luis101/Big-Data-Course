...
header = ['ÜBERSCHRIFT', 'TEXT', 'UPDATE']
...
for fn in args.files:
    ...
    for f in workfile:
        ...
        filessplit = [row.replace('\r\n', ' ').strip() for row in f.split('\r\n\r\n') if len(row) > 0 and 'Alle Rechte Vorbehalten' not in row]
        ...
        meta_dict['ÜBERSCHRIFT'] = ueb
        #meta_dict['TEXT']  = tex
        meta_dict['UPDATE']        = date
        ...
        if args.sep == 'csv':
            ...
            ## write date first for separate files
            fh.write('UPDATE: %s\n'  % meta_dict['UPDATE'])
